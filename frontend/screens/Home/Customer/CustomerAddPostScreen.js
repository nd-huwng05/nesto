...
export default function CustomerAddPostScreen({navigation, route}) {
    ...
    useEffect(() => {
        ...
        const loadBookingRecords = async () => {
            try {
                ...
                rows.forEach((item) => {
                    const bookingId = normalizeBookingId(item?.bookingCode || item?.booking_code || '');
                    const hotelName = String(item?.hotel_name || 'Hotel').trim();
                    const roomName = String(item?.roomType || item?.room_type || 'Room').trim();
    ...
    const handlePost = async () => {
        ...
        try {
            ...
            Alert.alert('Error', 'Unable to publish your post. Please try again.');
        }
    };
