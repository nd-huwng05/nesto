...
export default function CustomerReviewScreen({navigation, route}) {
    ...
    const loadPosts = async (showRefresh = false) => {
        ...
        try {
            setLoading(showRefresh ? false : true);
            const remoteRes = await fetchReviews({hotelName: scope.hotelName, roomName: scope.roomName});
            const remotePosts = remoteRes.status === 'success' && Array.isArray(remoteRes.data) ? remoteRes.data : [];
    ...
    useEffect(() => {
        setLoading(true);
        loadPosts(false);
    }, [scope.scopeKey, scope.hotelName, scope.roomName]);
