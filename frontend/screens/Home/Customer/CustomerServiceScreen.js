...
const categoryToTheme = (category) => {
    const key = String(category || '').toUpperCase();
    if (key.includes('SPA')) return {accent: '#7c4dff', icon: 'flower-outline'};
    if (key.includes('RESTAURANT')) return {accent: '#e67e22', icon: 'restaurant-outline'};
    if (key.includes('TRANSPORT') || key.includes('SHUTTLE')) return {accent: '#1e88e5', icon: 'car-sport-outline'};
    return {accent: '#2aa8b9', icon: 'sparkles-outline'};
};
...
export default function CustomerServiceScreen({navigation, route}) {
    ...
    useEffect(() => {
        ...
        (async () => {
            ...
            setServiceOptions(
                rows.map((row) => {
                    const theme = categoryToTheme(row?.category);
