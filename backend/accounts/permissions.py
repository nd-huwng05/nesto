...
class IsReceptionistMember(IsInAnyGroup):
    required_groups = ("Receptionist_Group", "Business_Group", "Admin_Group")
    ...
    def has_permission(self, request, view):
        ...
        if user.is_superuser:
            return True
        role = getattr(user, "role", None)
        if role in {"SUPER_ADMIN", "BUSINESS_OWNER", "RECEPTIONIST"}:
            return True
        return super().has_permission(request, view)


class IsHousekeepingMember(IsInAnyGroup):
    required_groups = ("Housekeeping_Group", "Business_Group", "Admin_Group")
    ...
    def has_permission(self, request, view):
        ...
        if user.is_superuser:
            return True
        role = getattr(user, "role", None)
        if role in {"SUPER_ADMIN", "BUSINESS_OWNER", "HOUSEKEEPING", "RECEPTIONIST"}:
            return True
        return super().has_permission(request, view)


class IsServiceMember(IsInAnyGroup):
    required_groups = ("Service_Group", "Business_Group", "Admin_Group")
    ...
    def has_permission(self, request, view):
        ...
        if user.is_superuser:
            return True
        role = getattr(user, "role", None)
        if role in {"SUPER_ADMIN", "BUSINESS_OWNER", "SERVICE"}:
            return True
        return super().has_permission(request, view)
